...
export default function BookingPage({ params }: { params: Params }) {
  const [bookedItem, setBookedItem] = useState<Search | undefined>(undefined);
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [ratingValue, setRatingValue] = useState<number | null>(null);
  const [show, setShow] = useState<boolean>(false);
  const [activeTab, setActiveTab] = useState<number>(0);
  const [bankShow, setBankShow] = useState<boolean>(false);
  const [gPay, setgPay] = useState<boolean>(false);
  const [pPay, setpPay] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch("../search.json")
      ...
      .then((data: Search[]) => {
        setBookedItem(
          data.find((item) => item.id === parseInt(params.booking))
        );
      });
  }, [params]);
                    ...
                    <div className='grid grid-cols-2 gap-7 items-end mt-4'>
                      <div>
                        <button
                          ...
                          className='w-full bg-[#1ab64f] cursor-pointer text-color4 text-base font-bold rounded py-[14px]'>
                          Pay Now
                        </button>
                      </div>
                      <div></div>
            ...
            <div className='flex'>
              <div className='space-y-2'>
                ...
                <div
                  onClick={() => {
                    openModal();
                    setRatingValue(bookedItem?.ratings ?? 0);
                  }}
                  ...
                  <div className='flex items-center gap-2 text-xs text-[#6d787d] font-normal'>
                    ...
                    <p>
                      {bookedItem && bookedItem.rate >= 4.0
                        ? "Vary Good"
                        : "Good"}
                    </p>
                  </div>
              ...
              <div>
                <Image
                  src={bookedItem?.image?.[0] || "/path/to/placeholder.jpg"}
                  width={100}
                  height={200}
                  alt=''
